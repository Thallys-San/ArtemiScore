...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (lockoutUntil && new Date() < new Date(lockoutUntil)) {
      ...
      setLoginError(`Muitas tentativas falhas. Tente novamente em ${remainingTime} minutos.`);
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      switch (error.code) {
        case "auth/invalid-credential":
          errorMessage = `Credenciais inválidas. Verifique seu email e senha. Tentativas restantes: ${10 - loginAttempts - 1}`;
          break;
        case "auth/invalid-email":
          errorMessage = `Email inválido. Por favor, insira um email válido. Tentativas restantes: ${10 - loginAttempts - 1}`;
          break;
        case "auth/user-disabled":
          errorMessage = "Esta conta foi desativada. Entre em contato com o suporte.";
          break;
        case "auth/user-not-found":
          errorMessage = `Nenhuma conta encontrada com este email. Tentativas restantes: ${10 - loginAttempts - 1}`;
          navigate("/RecuperarSenha", { state: { email } });
          break;
        case "auth/wrong-password":
          errorMessage = `Senha incorreta. Tente novamente ou redefina sua senha. Tentativas restantes: ${10 - loginAttempts - 1}`;
          break;
        ...
        case "auth/network-request-failed":
          errorMessage = `Problema de conexão. Verifique sua internet. Tentativas restantes: ${10 - loginAttempts - 1}`;
          break;
        default:
