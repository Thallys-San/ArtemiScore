  ...
  const Login = () => {
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        ...
        if (error.code === "auth/user-not-found") {
          setErro("O email utilizado não está cadastrado. Cadastre-se");
    ...
    const handleGoogleLogin = async () => {
      try {
        ...
        const token = await user.getIdToken();
        login (token, {uid:user.uid, email: user.email, displayName: user.displayName});
    ...
    const handlePasswordReset = async () => {
      ...
      try {
        ...
        if (methods.length === 0) {
          setEmailError("Este email não está cadastrado. Cadastre-se");
          return; // Não redireciona se o email não existir
        }

        // Se chegou aqui, o email existe e podemos redirecionar
