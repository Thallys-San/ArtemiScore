...
const Login = () => {
  ...
  const [resetStatus, setResetStatus] = useState(''); // mensagem de feedback reset
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate('/'); // ajuste essa rota se quiser ir para outra página
    } catch (error) {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      navigate('/'); // ajuste a rota se quiser
    } catch (error) {
    ...
    <section className="login-section">
      <div className="login-container">
        ...
        <form className="login-form" onSubmit={handleSubmit}>
          ...
          {resetStatus && (
            <p style={{ color: resetStatus.startsWith('Erro') ? 'red' : 'green', marginTop: '0.5rem' }}>
              {resetStatus}
            </p>
          )}
